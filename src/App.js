...
function App() {
  ...
  const fetchPokemonData = () => {
    if (!pokemonName.trim()) {
      alert('Por favor, ingrese un nombre de Pokémon válido.');
      return;
    }
  ...
  return (
    <div class ="container">
      ...
      <form onSubmit={handleSubmit}>
        <input type="text" class="form-control" value={pokemonName} onChange={handleChange} placeholder="Ingrese el nombre de un Pokémon"/>
      ...
      {data ? (
        <div class ="container">
          ...
          <h3>Número pokedex: {data.id}</h3>
          <h3>Altura: {data.height/10} Metros</h3>
          <h3>Peso: {data.weight/10} Kilos</h3>
          <h3>Habilidad N° 1: {data.abilities[0].ability.name}</h3>
